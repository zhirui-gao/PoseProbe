...
class Voxurf(torch.nn.Module):
    ...
    """
    def __init__(self, xyz_min, xyz_max, num_voxels=0, num_voxels_base=0,
                 alpha_init=None, nearest=False, mask_cache_path=None,
                 fast_color_thres=0,rgbnet_dim=0, rgbnet_direct=False, rgbnet_full_implicit=False,
                 rgbnet_depth=3, rgbnet_width=128,posbase_pe=5, viewbase_pe=4, geo_rgb_dim=3,
                 grad_mode='interpolate',s_ratio=2000, s_start=0.2, s_learn=False, step_start=0,
                 smooth_ksize=0, smooth_sigma=1, camera_noise=0., barf_c2f=None, i_train=[],
                 N_iters=20000, flow_ckpt_path='', flow_backbone='', HW=[[512, 512]], sg_config=None,
        ...
        self.grid_flat = self.grid[:, :, 1] * W + self.grid[:, :,
                                                  0]
        self.grid_flat = self.grid_flat.to(self.device).long()
        self.warp_network = DeformedImplicitField(
        ...
        self.mapping_scale = (2. / (self.xyz_max - self.xyz_min).max())
        ...
        if self.sdf_init_mode == "ball_init":
            ...
            self.sdf.grid.data = torch.from_numpy((x ** 2 + y ** 2 + z ** 2) ** 0.5 - 0.3).float()[None, None, ...].to(
                self.device)
            ...
            dist_x = np.minimum(np.abs(x - (rect_size[0] / 2 - center_point[0])),
                                np.abs(x - (rect_size[0] / 2 + center_point[0])))
            dist_y = np.minimum(np.abs(y - (rect_size[1] / 2 - center_point[1])),
                                np.abs(y - (rect_size[1] / 2 + center_point[1])))
            dist_z = np.minimum(np.abs(z - (rect_size[2] / 2 - center_point[2])),
                                np.abs(z - (rect_size[2] / 2 + center_point[2])))
    ...
    def init_sdf_from_density(self, smooth=False, reduce=1., ksize=3, sigma=1., zero2neg=True):
        ...
        if self.density.shape != self.sdf.grid.shape:
            self.density.data = F.interpolate(self.density.data, size=tuple(self.world_size), mode='trilinear',
                                              align_corners=True)
        if smooth:
    ...
    def density_total_variation(self, sdf_tv=0, smooth_grad_tv=0):
        cudnn.enabled = False
        tv = 0
        if sdf_tv > 0:
        ...
        if smooth_grad_tv > 0:
            smooth_tv_error = (
                        self.tv_smooth_conv(self.gradient.permute(1, 0, 2, 3, 4)).detach() - self.gradient.permute(1, 0,
                                                                                                                   2, 3,
                                                                                                                   4))
    ...
    def query_sdf_point_wocuda(self, rays_o, rays_d, global_step=None, keep_dim=False, return_depth=False,
                               use_deform=False,
                               **render_kwargs):
        # use_deform = False  # ToDO: delet
        ...
        if self.use_mapping_fun:
            # sdf_grid = self.activation(self.sdf_alpha) * \
            #            (1 / (1 + torch.exp(-self.activation(self.sdf_beta) * sdf_grid)) - 0.5)
            sdf_grid = self.activation(self.sdf_alpha) * (
                        torch.sigmoid(self.activation(self.sdf_beta) * sdf_grid) - 0.5)
        ...
        if use_deform:
            if self.bool_mapping_deformspace:
                deform, correction = self.warp_network(rays_pts_inbbox * self.mapping_scale)
                new_coords = deform / self.mapping_scale + rays_pts_inbbox
                correction = correction / self.mapping_scale
            else:
            ...
            sdf = self.grid_sampler(rays_pts_inbbox, sdf_grid)
    ...
    def query_sdf_point_wocuda_wodeform(self, rays_o, rays_d, global_step=None,
                                        keep_dim=False, return_depth=False, **render_kwargs):
        rays_pts, mask_outbbox, step, t_min, t_max = self.sample_ray_ori(rays_o=rays_o, rays_d=rays_d,
                                                                         is_train=global_step is not None,
                                                                         **render_kwargs)
    ...
    def query_sdf_point_wocuda_render(self, rays_o, rays_d, global_step=None, keep_dim=False,
                                      return_depth=False, use_deform=True, **render_kwargs):
        # use_deform = False  # ToDO: delete
        ...
        if use_deform and self.use_explict_deform:
            ...
            self.gradient = self.neus_sdf_gradient(sdf=sdf_grid)  # sdf_grid
            gradient = self.grid_sampler(ray_pts, self.gradient)

        elif use_deform and self.use_implict_deform:
            if self.bool_mapping_deformspace:
                deform, correction = self.warp_network(ray_pts * self.mapping_scale)
                new_coords = deform / self.mapping_scale + ray_pts
                correction = correction / self.mapping_scale
            else:
    ...
    def forward(self, rays_o, rays_d, viewdirs, use_deform=True, global_step=None, **render_kwargs):
        # use_deform = False  # ToDO: delete
        if global_step is None:
            self.progress.data.fill_(1.)
            self.warp_network.progress.data.fill_(1.)
        ...
        if self.use_mapping_fun:
            sdf_grid = self.activation(self.sdf_alpha) * (
                        torch.sigmoid(self.activation(self.sdf_beta) * sdf_grid) - 0.5)
        if use_deform and self.use_explict_deform:
            deform = self.grid_sampler(ray_pts, self.point_deform.grid)
            ray_pts = ray_pts + deform
            deform_xyz = self.grid_xyz + rearrange(self.point_deform.grid[0], 'c h w l -> h w l c', c=3)
            sdf_grid = self.grid_sampler(deform_xyz, sdf_grid)[None, None, ...]
            sdf_final = self.grid_sampler(ray_pts, sdf_grid)
            self.gradient = self.neus_sdf_gradient(sdf=sdf_grid)  # sdf_grid
            ...
            sdf_final = sdf + correction.squeeze()  # add correction
            sdf_deform = sdf_final - self.grid_sampler(ray_pts, sdf_grid, use_custom=True)
            if sdf_final.shape[0] == 0:
                gradient = torch.zeros((0, 3))
            else:
                gradient = \
        ...
        if self.use_deep_sdf:
            tv_sdf = self.density_total_variation(sdf_tv=0, smooth_grad_tv=0.05) + \
                     self.density_total_variation(sdf_tv=0.1, smooth_grad_tv=0.)

        k0_tv = self.k0_total_variation()
        ...
        if use_deform:
            ret_dict.update({
                'sdf_deform': sdf_deform,
                'grad_deform': grad_deform,
                'sdf_correct': correction
            })
        return ret_dict

    def inference(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        if global_step is None:
            self.progress.data.fill_(1.)
            self.warp_network.progress.data.fill_(1.)
        else:
    ...
    def extract_deform_geometry(self, bound_min, bound_max, resolution=128, threshold=0.0, **kwargs):
        ...
        def query_func(pts):
            if self.bool_mapping_deformspace:
                deform, correction = self.warp_network(self.mapping_scale * pts)
                new_coords = deform / self.mapping_scale + pts
                correction = correction / self.mapping_scale
            else:
...
def get_training_rays_semantic(rgb_tr_ori, mask_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, samplers):
    ...
    for c2w, img, mask, (H, W), K in zip(train_poses, rgb_tr_ori, mask_tr_ori, HW, Ks):
        ...
        rays_o, rays_d, viewdirs, xy_index = get_rays_of_a_view_semantic(H=H, W=W, K=K, c2w=c2w, ndc=ndc,
                                                                         inverse_y=inverse_y, flip_x=flip_x,
                                                                         flip_y=flip_y, background_sampler=background,
                                                                         object_sampler=object,
...
def get_training_rays_in_maskcache_sampling_grad(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, model,
                                                 render_kwargs):
    assert len(rgb_tr_ori) == len(train_poses) and len(rgb_tr_ori) == len(Ks) and len(rgb_tr_ori) == len(HW)
    CHUNK = 64
    DEVICE = rgb_tr_ori[0].device
