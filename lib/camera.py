...
from lib import util_barf
from lib.util_barf import log,debug
...
def get_center_and_ray_by_2d(points,intr,pose,mode='center'): # [HW,2]
    # given the intrinsic/extrinsic matrices,2d coordinate, get the camera center and ray directions]
    if mode == 'center':
        points = points + 0.5
    grid_3D = img2cam(to_hom(points), intr)  # [B,HW,3]
    center_3D = torch.zeros_like(grid_3D)  # [B,HW,3]
    # transform from camera to world coordinates
    grid_3D = cam2world(grid_3D, pose)  # [B,HW,3]
    center_3D = cam2world(center_3D, pose)  # [B,HW,3]
...
def get_novel_view_poses(opt,pose_anchor,N=60,scale=1):
    ...
    pose_novel = pose.compose([pose_oscil,pose_anchor[None]])
    return pose_novel
