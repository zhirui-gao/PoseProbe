...
from lib.bg_nerf.source.datasets.data_utils import get_nearest_pose_ids
from lib.bg_nerf.source.training.core.sampling_strategies import sample_rays
...
from lib.bg_nerf.source.utils.geometry.batched_geometry_utils import (batch_backproject_to_3d, batch_project)
from lib.bg_nerf.source.training.core.base_losses import BaseLoss
...
class DepthConsistencyLoss(BaseLoss):
    ...
    def compute_loss(self, opt: Dict[str, Any], data_dict: Dict[str, Any],
        ...
        pixels_ref, _ = sample_rays(H, W, nbr=max(256, self.opt.nerf.rand_rays),
        ...
        # depth_con = ret_ref.density_samples[...,:10].mean()
        ...
        if compute_fine_sampling:
            ...
            # depth_con += ret_ref.density_samples_fine[...,:10].mean()
        else:
        ...
        # loss_dict.update({'depth_cons': depth_con})
        ...
        loss, stats_dict, plotting_dict= self.compute_loss_at_sampled_pose(data_dict, pose_w2c_at_unseen=pose_w2c_at_sampled, intr_at_unseen=intr_at_sampled,
                                                 pseudo_gt_3dpts_in_w=pts3d_in_w_from_ref, H=H, W=W,
                                                 pixels_in_ref=pixels_ref)

        # if self.opt.loss_weight.fea_cons is not None:
        #     id_ref = (id_self + random.choice([1, -1])) % (poses_c2w.shape[0])
        #     loss_, stats_dict_, plotting_dict_= self.compute_loss_at_feature_ref(data_dict, poses_w2c[id_ref], intr[id_ref], pseudo_gt_3dpts_in_w=pts3d_in_w_from_ref,
        #                                 H=H, W=W, pixels_in_self=pixels_ref, self_id=id_self, other_id=id_ref)
    ...
    def compute_loss_at_feature_ref(self, data_dict: Dict[str, Any], pose_w2c_ref: torch.Tensor, intr_ref: torch.Tensor,
                                    pseudo_gt_3dpts_in_w: torch.Tensor,
                                    H: int, W: int, pixels_in_self: torch.Tensor, self_id: int, other_id:int,
                                    )-> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
        iteration = data_dict['iter']
        stats_dict, plotting_dict = {'fea_px': pseudo_gt_3dpts_in_w.shape[0]}, {}

        # project pseudo gt 3D points to unseen view ==> creates pseudo-gt depth.
        pts_in_sampled_img, pseudo_gt_depth_in_sampled_img = batch_project(pseudo_gt_3dpts_in_w,
                                                                           T_itoj=pose_w2c_ref, Kj=intr_ref,
                                                                           return_depth=True)
        # (N, 2)
        ...
        pts_in_sampled_img = pts_in_sampled_img[valid_pts]
        pixels_in_self = pixels_in_self[valid_pts]
        pseudo_gt_depth_in_sampled_img = pseudo_gt_depth_in_sampled_img[valid_pts]

        if pseudo_gt_depth_in_sampled_img.shape[0] == 0:
            return {'fea_cons': torch.tensor(0., requires_grad=True).to(self.device)}, {}, {}

        # compute visibility mask
        with torch.no_grad():
            ret_max_depth_at_sampled = self.net.render_up_to_maxdepth_at_specific_pose_and_rays \
                (self.opt, data_dict, pose_w2c_ref[:3], intr_ref, H, W,
                 depth_max=pseudo_gt_depth_in_sampled_img, pixels=pts_in_sampled_img,
                 mode='train', iter=iteration)
            visibility_weight_ = ret_max_depth_at_sampled['all_cumulated_fine'].squeeze(0).unsqueeze(-1) if \
                'all_cumulated_fine' in ret_max_depth_at_sampled.keys() else \
        ...
        pts_in_sampled_img = pts_in_sampled_img[valid_mask]
        pixels_in_self = pixels_in_self[valid_mask]
        # pseudo_gt_depth_in_sampled_img = pseudo_gt_depth_in_sampled_img[valid_mask]
        visibility_weight = visibility_weight_[valid_mask]

        if visibility_weight.shape[0] == 0:
            return {'fea_cons': torch.tensor(0., requires_grad=True).to(self.device)}, {}, {}

        # coarse rendering


        norn_p = torch.stack([pixels_in_self, pts_in_sampled_img])
        norn_p[:, :, 0] = 2. * norn_p[:, :, 0] / W - 1
        norn_p[:, :, 1] = 2. * norn_p[:, :, 1] / H - 1

        proj_fea = common.get_tensor_values(data_dict.vgg_features[[self_id, other_id], ...],
                                                       norn_p,mode='bilinear', scale=False, detach=False,
                                                       detach_p=False, align_corners=True)

        sim = torch.cosine_similarity(proj_fea[0, ...], proj_fea[1, ...], dim=-1)
        loss_fea = torch.mean(((1 - sim) ** 2) * visibility_weight)

        stats_dict['fea_vis_weight'] = visibility_weight.sum() / (visibility_weight.nelement() + 1e-6)

        loss = {'fea_cons':  loss_fea}

        # # compute nerf predictions at the projected pixel locations, with valid visibility mask.
        # ret_self = self.net.render_image_at_specific_pose_and_rays(self.opt, data_dict, pose_w2c_ref[:3],
        #                                                            intr_ref, H, W,
        #                                                            pixels=pts_in_sampled_img, mode='train',
        #                                                            iter=iteration)
        # depth_rendered_at_sampled = ret_self.depth.squeeze().reshape(-1)  # (N)
        # accuracy = ret_self.opacity.squeeze(0).detach()
        # visibility_weight = visibility_weight * accuracy
        # loss_sampling = self.compute_diff_loss \
        #     (self.opt.diff_loss_type,
        #      diff=pseudo_gt_depth_in_sampled_img.view(-1) - depth_rendered_at_sampled.view(-1),
        #      weights=visibility_weight.view(-1))
        ...
        #
        # stats_dict['avg_vis_weight'] = visibility_weight.sum() / (visibility_weight.nelement() + 1e-6)
        # loss.update({'depth_cons': loss_sampling})
        ...
        if self.opt.gradually_decrease_depth_cons_loss:
            ...
            gamma = 2 ** (iteration // self.opt.depth_cons_loss_reduct_at_x_iter)
            # loss['depth_cons'] = loss['depth_cons'] / gamma
            loss['fea_cons'] = loss['fea_cons'] / gamma
