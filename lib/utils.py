...
import torch
from lib.bg_nerf.source.utils.torch import exponentiel_lr_starting_at_x
import cv2
...
def create_optimizer_or_freeze_matcher(model, cfg_train, global_step):
    decay_steps = cfg_train.lrate_decay * 1000
    decay_factor = 0.1 ** (global_step / decay_steps)
    param_group = []
    for k in cfg_train.keys():
        if not k.startswith('matcher_lrate_'):
            continue
        k = k[len('matcher_lrate_'):]
        ...
        if param is None:
            ...
            continue
        lr = getattr(cfg_train, f'matcher_lrate_{k}') * decay_factor
        if lr > 0:
        ...
        return Adam(param_group, betas=(0.9, 0.999))

def freeze_nerf(model,cfg_train):
    for k in cfg_train.keys():
        ...
        if isinstance(param,torch.nn.Parameter):
            ...
            for para in param.parameters():
                para.requires_grad = False

def unfreeze_nerf(model,cfg_train):
    for k in cfg_train.keys():
        if not k.startswith('lrate_'):
            continue
        k = k[len('lrate_'):]
...
def freeze_matching(model,cfg_train):
    for k in cfg_train.keys():
        if not k.startswith('matcher_lrate_'):
            continue
        k = k[len('matcher_lrate_'):]
        if not hasattr(model, k):
            continue
        param = getattr(model, k).superglue
        for para in param.parameters():
            para.requires_grad = False

def unfreeze_matching(model,cfg_train):
    for k in cfg_train.keys():
        if not k.startswith('matcher_lrate_'):
            continue
        k = k[len('matcher_lrate_'):]
        if not hasattr(model, k):
            continue
        param = getattr(model, k).superglue
        for name, para in param.named_parameters():
            para.requires_grad = True

def setup_optimizer_barf(model, opt):
    optimizer = getattr(torch.optim, opt.optim.algo)
    optim = optimizer(
        [dict(params=model.parameters(), lr=opt.optim.lr)]
    )
    ...
    if opt.optim.sched:
        ...
        sched = scheduler(optim, **kwargs)
    return optim, sched


def setup_optimizer(model, opt):
    optim = torch.optim.Adam(params=model.nerf.parameters(),
                             lr=opt.optim.lr, betas=(0.9, 0.999))
    if opt.nerf.fine_sampling:
        optim.add_param_group(dict(params=model.nerf_fine.parameters(),
                                   lr=opt.optim.lr, betas=(0.9, 0.999)))
    # set up scheduler
    if opt.optim.sched:
        if opt.optim.start_decrease > 0:
            sched = exponentiel_lr_starting_at_x(opt, optim)
        else:
            ...
            if opt.optim.lr_end:
                ...
                print(max_iter)
            kwargs = {k: v for k, v in opt.optim.sched.items() if k != "type"}
            sched = scheduler(optimizer=optim, **kwargs)
    return optim, sched
...
def create_optimizer_pose_mip(model):
    optim_pose = Adam([dict(params=model.se3_refine, lr=1e-5)])
    return optim_pose
def create_optimizer_pose(model, cfg_train,max_iter=1, align=False, index=None):
    if align is False:
        optim_pose = Adam([dict(params=model.se3_refine, lr=cfg_train.lr_pose)])
    else:
        optim_pose = Adam([dict(params=model.se3_align_refine, lr=1e-5)])  # cfg_train.lr_pose*0.1
        # optim_pose = torch.optim.SGD([dict(params=model.se3_refine[index], lr=cfg_train.lr_pose)])
    if cfg_train.sched_pose is None:
        return optim_pose, None
    else:
        # set up scheduler
        scheduler = getattr(torch.optim.lr_scheduler, cfg_train.sched_pose)
        assert (cfg_train.sched_pose == "ExponentialLR")
        gamma = (cfg_train.lr_pose_end / (1e-10+cfg_train.lr_pose)) ** (1. / max_iter)
        kwargs = {'gamma':gamma}
        sched_pose = scheduler(optim_pose, **kwargs)
        return optim_pose, sched_pose
''' Checkpoint utils
'''
...
def load_checkpoint(model, optimizer, ckpt_path, no_reload_optimizer, strict=True):
    ...
    current_pose = torch.load(ckpt_path)
    start = ckpt['global_step']
    if 'rgbnet.0.weight' in ckpt['model_state_dict']:
    ...
    return model, optimizer, start,current_pose
...
def load_model(model_class, ckpt_path, new_kwargs=None, strict=False):
    ...
    optimized_poses = ckpt['current_pose']
    try:
