...
from lib.bg_nerf.source.training.core.correspondence_utils import (generate_pair_list,
...
class CorrespondenceBasedLoss(BaseLoss, CorrrespondenceUtils):
    ...
    def __init__(self, opt: Dict[str, Any], nerf_net: torch.nn.Module, flow_net: torch.nn.Module, 
        ...
        if 'depth_gt' in train_data.keys():
    ...
    def compute_correspondences(self, train_data: Dict[str, Any]):
        ...
        images = train_data['image']  # (N_views, 3, H, W)
        H, W = images.shape[-2:]
        poses = train_data['pose']  # ground-truth poses w2c
        ...
        flow_plot = self.flow_net.visualize_mapping_combinations(images=train_data.image,
